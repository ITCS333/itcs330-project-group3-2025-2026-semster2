...
function getCommentsByAssignment(PDO $db, $assignmentId): void
{
    ...
    $statement = $db->prepare("SELECT id, assignment_id, author, text, created_at FROM comments_assignment 
...
function createComment(PDO $db, array $data): void
{
    ...
    if ($text === ''){
        sendResponse(['success'=> false, 'message'=>'Comment text is required'], 400);
    }
    ...
    //       VALUES (?, ?, ?)
    $statement = $db->prepare("INSERT INTO comments_assignment (assignment_id, author, text) VALUES (?, ?, ?)");
    ...
    // Otherwise sendResponse HTTP 500.
        if ($statement->rowCount() > 0){
            ...
            $getComment= $db->prepare("SELECT id, assignment_id, author, text, created_at FROM comments_assignment WHERE id = ?");
            $getComment->execute([$newId]);
            $comment= $getComment->fetch(PDO::FETCH_ASSOC);
        sendResponse(['success'=> true, 'message'=>'Comment created', 'id'=> (int)$newId, 'data'=> $comment], 201);
    } else{
        sendResponse(['success'=> false, 'message'=>'Could not create comment'], 500);        
    }
}
...
function deleteComment(PDO $db, $commentId): void
{
    ...
    if ($statement->rowCount() > 0){
        ...
        sendResponse(['success'=> false, 'message'=>'Could not delete comment'], 500);        
    }
}
